...
def get_board_cords_in_screen():
    ...
    texto=font.render('press "z" to define one corner and again to define the rectangle', True, white, blue)
    while not done:
    ...
    return min(n[0],m[0]),min(n[1],m[1]),abs(m[0]-n[0]),abs(m[1]-n[1])


def screenshot(cords):
    a,b,c,d = cords
    pyautogui.screenshot("board.png",region=(a,b,c,d))
